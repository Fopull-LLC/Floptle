...
second machine, a screenshot of a game that builds its own world, and errors
that named Lua instead of the cause. There is also a prebuilt relay for small
Linux servers, coloured text, and a trigger flag you can afford again.
...
never the problem: trigger overlap asked every physics body in the world for an
exact distance to that mesh, at any range.
