...
Everything here comes from building a real matchmaking flow and finding the
places where the engine went quiet.
...
- **`scripts/setup-relay.sh`** — one command to stand up a relay on a fresh cloud
  box. Detects `apt` vs `dnf`, opens the firewall the right way for either, and
  adds swap when it lands on a 1 GB instance (where the build otherwise gets
  killed with nothing about memory anywhere in the output).
- **`cargo run -p floptle-net --example relay_probe -- host:port`** completes a
  ...
  that.
- **A correction to v0.10.1's notes.** They said the gamepad half of the joiner
  bug was narrower than reported. That was wrong — a joiner's controller drove
  nothing, exactly as first reported. It's asserted by a test now rather than
