    ...
    pub(crate) fn scene_ui(&mut self, ui: &mut egui::Ui, game: bool) {
        ...
        if !game && !self.rig_gizmos.is_empty() {
            ...
            for r in self.rig_gizmos {
                // A bone whose head is the skeleton root never reaches
                // `r.bones` at all (see `viz::is_root_bone`) — so every bone
                // drawn here is a real one, regardless of what's selected,
                // and always draws the same way.
                for b in &r.bones {
