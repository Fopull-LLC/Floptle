...
const LUA_API: &[ApiEntry] = &[
    ...
    ApiEntry { label: "draw.line", insert: "draw.line(", doc: "draw.line(x1,y1,z1, x2,y2,z2, r,g,b [, a]) — queue one world-space 3D line for THIS tick (immediate mode: re-draw every fixedUpdate while wanted). Depth-tested in the scene, so bodies occlude it — the KSP-style map draws its orbit conics with these." },
