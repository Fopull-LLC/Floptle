...
const LUA_API: &[ApiEntry] = &[
    ...
    ApiEntry { label: "terrain.flush", insert: "terrain.flush()", doc: "terrain.flush() — write every EDITED resident terrain field to the save slot now (terrain.saveDir must be set). Call at checkpoints / exit-to-menu so the slot reloads from fast files instead of regenerating; streaming already flushes when bodies stream out." },
    ...
    ApiEntry { label: "node:setCelestial", insert: ":setCelestial{", doc: "node:setCelestial{mu=…, bodyRadius=…, soi=0, parent=\"Sun\", a=…, e=…, i=…, m0=…, atmoColor={r,g,b}, atmoHeight=…, atmoDensity=…, clouds=…, luminosity=…, starColor={r,g,b}} — set (creating if absent) the node's CelestialBody. camelCase fields; colors take {r,g,b}." },
