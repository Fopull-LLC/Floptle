...
const LUA_API: &[ApiEntry] = &[
    ...
    ApiEntry { label: "vec3.x", insert: "vec3.x", doc: "The vector's X. Vectors are values, not handles: nothing you do to one reaches whatever it came from. Whether you can WRITE it depends on the project's vec3 (Project Settings → Script vec3): under `exact` (every project made before the setting existed) `v.x = 5` changes that vector; under `fast` (new projects) a vector cannot be assigned into and `v.x = 5` raises, naming the fix — `v = v:withX(5)`, which works on both. Nodes are unaffected either way: `node.x = 5` moves the node." },
