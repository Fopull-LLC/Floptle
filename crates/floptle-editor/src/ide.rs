...
const LUA_API: &[ApiEntry] = &[
    ...
    // ---- steam (docs/steam-integration-proposal.md) ----
    ...
    ApiEntry { label: "steam.onPersonaChanged", insert: "steam.onPersonaChanged(function()\n  \nend)", doc: "steam.onPersonaChanged(fn) — fires once when the local user's persona (name or avatar) changes. Re-read steam.personaName() from inside it; avatars aren't exposed to Lua yet (no engine primitive turns raw bytes into a drawable texture at runtime — see docs/steam-integration-proposal.md)." },
    ...
    ApiEntry { label: "steam", insert: "steam", doc: "Steam integration: identity, app/build info, and steam.available() for branching. Always present so steam.available() is always safe to call — nil (not an error) is what every other steam.* getter answers when it's false. See docs/steam-integration-proposal.md." },
    ...
    ApiEntry { label: "nav.settings", insert: "nav.settings()", doc: "nav.settings() — the character the mesh was baked for: radius, height, maxSlope, stepHeight, cellSize, plus areaCount and area (square metres of walkable ground). A script moving a body along a path needs the radius the mesh was eroded by; guessing it is how a character ends up scraping the wall the erosion existed to avoid." },
    ...
    ApiEntry { label: "nav.offLinks", insert: "nav.offLinks()", doc: "nav.offLinks() — every Nav Link in the level as data: { id, name, from, to, bidirectional, cost, duration, enabled, ground }, world space. The ladders, jumps and doors somebody placed, as opposed to nav.links(), which is the thousands of portals the bake derived between neighbouring areas. Two things called links is inherited and worth knowing before reading either. Use nav.link(name, open) to change one; this only reads. nil with no bake." },
