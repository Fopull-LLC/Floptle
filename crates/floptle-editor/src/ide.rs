...
const COMPONENT_FIELDS: &[(&str, &str, &str)] = &[
    ...
    ("PostProcess", "exposure", "Colour grade: exposure in stops. 0 = unchanged, +1 = twice the light."),
    ("PostProcess", "contrast", "Contrast about 18% grey. 1 = unchanged."),
    ("PostProcess", "saturation", "Saturation. 1 = unchanged, 0 = greyscale."),
    ("PostProcess", "temperature", "White balance, blue to amber. 0 = unchanged."),
    ("PostProcess", "tint", "White balance, green to magenta. 0 = unchanged."),
    ("PostProcess", "lift", "Lift the black floor. 0 = unchanged."),
    ("PostProcess", "gradeGamma", "Midtone gamma. 1 = unchanged."),
    ("PostProcess", "gain", "Scale the highlights. 1 = unchanged."),
    ("PostProcess", "aberration", "Chromatic aberration: red and blue drift apart toward the edges. 0 = off."),
    ("PostProcess", "distortion", "Lens distortion: positive barrels, negative pincushions. 0 = off."),
    ("PostProcess", "grain", "Film grain amount, 0..1. 0 = off."),
...
ApiEntry { label: "net.notice", insert: "net.notice()", doc: "net.notice() — what the relay last told this HOST about the session, or nil. Today there is one: your account is at its player ceiling, so new joins are being turned away — and nobody already playing was disconnected. Said once per episode, not once per refused join.\n\nWorth putting on your own lobby screen. The people it concerns are the friends who cannot get in, and they are looking at your game rather than at your console; \"we are full right now, someone will be along\" is a far better experience than a join that silently fails. nil on a client and on a direct/LAN host, where there is no relay to say anything." },
    ...
    // ---- steam (the Steam integration plan) ----
    ApiEntry { label: "steam.available", insert: "steam.available()", doc: "steam.available() — true only for a floptle run/exported/served session with a real Steam client initialized. false in the editor's own docked Play-mode viewport, in every other session, and whenever no Steam client is running — branch on this before any other steam.* call, none of which raise when it's false (they answer nil)." },
    ApiEntry { label: "steam.localUserId", insert: "steam.localUserId()", doc: "steam.localUserId() — the signed-in local user's SteamID64, as a STRING (it exceeds what an f64 represents exactly). nil when steam.available() is false." },
    ApiEntry { label: "steam.personaName", insert: "steam.personaName()", doc: "steam.personaName() — the local user's current display name. nil when steam.available() is false." },
    ApiEntry { label: "steam.buildId", insert: "steam.buildId()", doc: "steam.buildId() — this build's Steam build id. nil when steam.available() is false." },
    ApiEntry { label: "steam.installDir", insert: "steam.installDir()", doc: "steam.installDir() — this app's install directory, as Steam reports it. nil when steam.available() is false." },
    ApiEntry { label: "steam.betaName", insert: "steam.betaName()", doc: "steam.betaName() — the beta branch this build was installed from. nil on the default branch, and nil when steam.available() is false." },
    ApiEntry { label: "steam.isFamilyShared", insert: "steam.isFamilyShared()", doc: "steam.isFamilyShared() — true if this app is being played on a license borrowed from another account (Steam Family Sharing), not one the signed-in user owns. nil when steam.available() is false." },
    ApiEntry { label: "steam.isCybercafe", insert: "steam.isCybercafe()", doc: "steam.isCybercafe() — true if Steam has flagged this as a cybercafe/shared-computer license. nil when steam.available() is false." },
    ApiEntry { label: "steam.uiLanguage", insert: "steam.uiLanguage()", doc: "steam.uiLanguage() — Steam's own UI language right now (e.g. \"english\", \"french\") — a reasonable default for your own localization. nil when steam.available() is false." },
    ApiEntry { label: "steam.isSteamDeck", insert: "steam.isSteamDeck()", doc: "steam.isSteamDeck() — true if this session is running on Steam's own handheld hardware. Assume no physical keyboard/mouse when true. nil when steam.available() is false." },
    ApiEntry { label: "steam.isBigPictureMode", insert: "steam.isBigPictureMode()", doc: "steam.isBigPictureMode() — true if Steam's own full-screen \"10-foot\" mode is active. nil when steam.available() is false." },
    ApiEntry { label: "steam.onPersonaChanged", insert: "steam.onPersonaChanged(function()\n  \nend)", doc: "steam.onPersonaChanged(fn) — fires once when the local user's persona (name or avatar) changes. Re-read steam.personaName() from inside it; avatars aren't exposed to Lua yet (Steam hands out raw pixels, not an image file, so they need a binding of their own — see the Steam integration plan)." },
    ...
    ApiEntry { label: "steam.findLeaderboard", insert: "steam.findLeaderboard(\"HIGH_SCORES\", function(board, err) end)", doc: "steam.findLeaderboard(name, cb) — looks a leaderboard up by name. cb(board, err) runs on a LATER frame, exactly once, in every session — including when there is no Steam at all, where err says so. board is { id, name, entryCount, sort, display }, or nil with NO err when there simply is no board by that name (a normal answer to branch on, not a failure). board.id is a STRING and lasts only this session — find the board again next run rather than saving it." },
    ApiEntry { label: "steam.findOrCreateLeaderboard", insert: "steam.findOrCreateLeaderboard(\"HIGH_SCORES\", { sort = \"descending\", display = \"numeric\" }, function(board, err) end)", doc: "steam.findOrCreateLeaderboard(name, opts, cb) — like steam.findLeaderboard, but creates the board if it doesn't exist. opts (optional): sort = \"descending\" (default; higher is better) or \"ascending\" (lower is better, for times); display = \"numeric\" (default), \"seconds\" or \"milliseconds\". Creating boards this way is a development convenience — a shipping game's boards are normally declared on the Steamworks admin site, where they can also be reset and moderated." },
    ApiEntry { label: "steam.uploadScore", insert: "steam.uploadScore(board.id, 1200, function(res, err) end)", doc: "steam.uploadScore(boardId, score, opts, cb) — uploads a score for the local user. opts (optional): method = \"keepBest\" (default — keeps whichever score ranks better) or \"forceUpdate\" (overwrite even with a worse one, for a \"most recent run\" board); details = a list of whole numbers stored alongside the score (ghost data, a replay seed), up to 64. cb(res, err) with res = { score, changed, rank, previousRank } — score is what is STORED, which under keepBest is not necessarily what you uploaded, and changed is how a \"new personal best!\" banner knows." },
    ApiEntry { label: "steam.downloadScores", insert: "steam.downloadScores(board.id, { scope = \"global\", start = 1, count = 10 }, function(rows, err) end)", doc: "steam.downloadScores(boardId, opts, cb) — downloads leaderboard rows. opts (optional): scope = \"global\" (default, ranks from the top), \"friends\" (only your friends) or \"aroundUser\" (ranks RELATIVE to your own — start = -4 with count = 9 gives you plus the four either side); start (default 1) and count (default 10). cb(rows, err) with rows a list of { userId, rank, score, details }; userId is a STRING and details is an empty list when none was uploaded." },
    ApiEntry { label: "steam.leaderboardsInFlight", insert: "steam.leaderboardsInFlight()", doc: "steam.leaderboardsInFlight() — how many leaderboard requests have been made and not yet called back. What a \"loading scores…\" spinner hangs off. Pressing Stop drops every pending callback, so this returns to 0." },
    ApiEntry { label: "steam.createLobby", insert: "steam.createLobby({ kind = \"public\", maxMembers = 8 }, function(lobby, err) end)", doc: "steam.createLobby(opts, cb) — creates a lobby and puts you in it. A lobby is DISCOVERY, not transport: it's how players find each other and agree what they're about to play, and it decides nothing about how the game's packets travel. opts (optional): kind = \"public\" (default; anyone can find it), \"friendsOnly\", \"private\" (invite only) or \"invisible\" (joinable by id but never returned by a search); maxMembers = 8 by default, 1 to 250. cb(lobby, err) on a later frame, exactly once — lobby is { id, memberCount, memberLimit, owner, data }." },
    ApiEntry { label: "steam.joinLobby", insert: "steam.joinLobby(id, function(lobby, err) end)", doc: "steam.joinLobby(id, cb) — joins a lobby by id (a string). cb(lobby, err) exactly once, on a later frame. When it fails, err is a real reason where Steam gives one — \"that lobby is full\", \"you're banned from that lobby\", \"that lobby no longer exists\" — worth showing the player as-is." },
    ...
    ApiEntry { label: "node.vel", insert: "node.vel", doc: "The body's velocity as a vec3 (read/write). `node.vel = node.vel + node.up * jump` replaces three vx/vy/vz lines, and it accepts anything with x/y/z. A body's fields are there from the first tick its scripts run — a rig that net.spawn just handed you included; nil only on a node with no RigidBody." },
    ...
    ApiEntry { label: "camera.screenSize", insert: "camera.screenSize()", doc: "camera.screenSize() → w, h — the game viewport size in pixels. camera.exists() is true once a live game camera is being fed. Under `floptle shot` this is the picture's --size from the first update, so a HUD can be checked at any screen size headless; `floptle run` renders nothing and reports no camera." },
    ApiEntry { label: "camera.pixelsPerUnit", insert: "camera.pixelsPerUnit()", doc: "camera.pixelsPerUnit([distance]) → px — how many screen pixels one world unit covers. The number every 2D game used to derive by hand from the FOV and the camera's Z, and then snap the camera to a multiple of for crisp pixels.\n\nUnder an ORTHOGRAPHIC camera the answer is the same everywhere and `distance` is ignored — that is what an orthographic projection means, and it is the case a flat game is in. Under a perspective one it is measured at `distance`, defaulting to the camera's distance from the origin.\n\nA 2D camera can do the snapping for you: node:setCamera2D{ pixelSnap = 32 }." },
    ...
    ApiEntry { label: "terrain.busy", insert: "terrain.busy()", doc: "terrain.busy() — is the background terrain worker already occupied? True while any field is generating or streaming in. Whole-body fills and residency streaming share one background budget, so a game that BUILDS ITS WORLD AS THE PLAYER TRAVELS should ask before queueing the next one — otherwise the new world goes in behind the ground somebody is standing on. The pattern: build one thing, wait for this to go quiet, build the next." },
    ...
    ApiEntry { label: "save.set", insert: "save.set(", doc: "save.set(\"gold\", 42) — store persistent game data: survives Play sessions, editor restarts, and ships with exported builds. Values follow the synced-var guardrails (numbers/strings/bools/tables, depth <= 4, <= 1 KB), and a slot holds at most 10 000 keys / 4 MB in all. Flushed on Stop + every few seconds during Play." },
    ...
    ApiEntry { label: "assets", insert: "assets", doc: "Reference files under Assets/ in code: assets.getFile(path), assets.getContents(dir), assets.preload(models, cb). Read and write your own data files: assets.readText / writeText, assets.readJson / writeJson." },
    ...
    ApiEntry { label: "assets.textureFromUrl", insert: "assets.textureFromUrl(", doc: "assets.textureFromUrl(url [, opts], function(tex, err) end) — download a picture (a profile picture, a shared image) and make it a texture. `tex` is a name like \"img:3\" that goes anywhere a texture path does: draw.quad, a UI image's texture, a material's texture; a UI image with radius = half its size draws round. PNG, JPEG and WebP only, told apart by the bytes' own signature, at most 4096 pixels a side; anything else answers nil, why. The download is exactly http.get (same opts, Play only, public addresses, the same rate limits), and a URL already loaded this session answers from memory. The callback runs on a later frame. Kept until assets.release(tex) or Stop. A host that draws nothing (a dedicated server) answers with an error and downloads nothing. Refuses: what http.get refuses, at the call." },
    ApiEntry { label: "assets.textureFromBytes", insert: "assets.textureFromBytes(", doc: "assets.textureFromBytes(bytes, function(tex, err) end) — assets.textureFromUrl for bytes you already have, such as a blob's res.body. Same formats, limits and names; at most 8 MB of bytes." },
    ApiEntry { label: "assets.release", insert: "assets.release(", doc: "assets.release(tex) -> bool — let go of a texture made by assets.textureFromUrl / textureFromBytes; anything still drawing it draws nothing. true if it was one of those; false for anything else (a project texture is never released)." },
    ...
    ApiEntry { label: "account.state", insert: "account.state()", doc: "account.state() — \"signedOut\" | \"starting\" | \"waiting\" | \"signedIn\" | \"failed\". Polled rather than called back, because signing in takes as long as a person takes to pick up their phone and a sign-in screen is redrawing anyway. The first ask in a run reads the stored session (the Hub's, or this game's last sign-in) with no Cloud call, and answers \"starting\" until it has, so a Sign in button shown only on \"signedOut\" never flashes at a signed-in player." },
    ...
    ApiEntry { label: "app", insert: "app", doc: "The settings a game offers the person playing it, and the one thing every main menu needs: quit. app.quit(), app.title(), app.version(), and the video settings — app.vsync/setVsync, app.retro/setRetro, app.retroHeight/setRetroHeight, app.retroIntegerScale/setRetroIntegerScale. A setting you change here is for THIS SESSION: it lives in project.ron, the file that ships to every player, so Stop puts it back — persist the player's choice yourself with save.*, the same rule access.* follows. Window resolution and fullscreen are NOT here yet; that is one question (windowing) waiting to be answered properly. See docs/scripting.md §30." },
    ApiEntry { label: "app.quit", insert: "app.quit()", doc: "app.quit() — end the game. What that means depends on where it is running, and they are genuinely different things: an EXPORTED BUILD closes (your save.* data is flushed first, since somebody quitting from a settings menu expects the setting they just changed to have been kept); the EDITOR stops Play and says so in the Console, deliberately NOT a process exit — an editor that closed because a game under test called quit would take your unsaved work with it; `floptle run` ends the run where it stands and reports that it stopped early." },
    ApiEntry { label: "app.title", insert: "app.title()", doc: "app.title() — the game's title, for a menu to put at the top of itself. The export manifest's title in a build, the project's otherwise, and the project folder's name when neither is set — the same chain the window title uses, so a menu and the title bar never disagree." },
    ApiEntry { label: "app.version", insert: "app.version()", doc: "app.version() — the engine version this build was made with, e.g. \"0.81.0\". For an about box, and for a bug report to quote." },
    ApiEntry { label: "app.vsync", insert: "app.vsync()", doc: "app.vsync() → the current frame pacing as a name: \"On\", \"Adaptive\" or \"Off\" — the same names project.ron uses, so a Video tab can show what is set and save it straight back." },
    ApiEntry { label: "app.setVsync", insert: "app.setVsync(", doc: "app.setVsync(\"Adaptive\") — set the frame pacing. \"On\" is classic vsync; \"Adaptive\" renders freely and lets the display take the newest frame; \"Off\" presents the instant a frame is ready, tearing and all. A name it does not recognise RAISES and lists the three — a control that silently kept the old value is a control that appears to work. For this session only: Stop puts the project back, so persist the choice with save.*." },
    ApiEntry { label: "app.retro", insert: "app.retro()", doc: "app.retro() → whether the retro presentation is on: the game composites at a small internal resolution and upscales, which is what gives it chunky pixels." },
    ApiEntry { label: "app.setRetro", insert: "app.setRetro(", doc: "app.setRetro(false) — turn the retro presentation off (or on), so the game composites at the window's own resolution instead. For this session only; persist it with save.*." },
    ApiEntry { label: "app.retroHeight", insert: "app.retroHeight()", doc: "app.retroHeight() → the internal height the game composites at, in pixels. In a pixel-art game this IS the resolution setting a player means — the window size is a separate question the engine does not answer yet." },
    ApiEntry { label: "app.setRetroHeight", insert: "app.setRetroHeight(", doc: "app.setRetroHeight(360) — set the internal height the game composites at, 32–4320 pixels. Outside that range RAISES rather than clamping, the way access.setTextScale does: a settings slider hands over a number it already bounded. For this session only; persist it with save.*." },
    ApiEntry { label: "app.retroIntegerScale", insert: "app.retroIntegerScale()", doc: "app.retroIntegerScale() → whether the retro composite is upscaled by a WHOLE number and letterboxed rather than stretched to fill." },
    ApiEntry { label: "app.fullscreen", insert: "app.fullscreen()", doc: "app.fullscreen() → whether the game's window covers the screen. The real state, not the last thing asked for — so a Video tab shows what the player sees, including after F11 or Alt+Enter, which a build answers on its own." },
    ApiEntry { label: "app.setFullscreen", insert: "app.setFullscreen(", doc: "app.setFullscreen(true) — cover the screen (borderless, on the monitor the window is on; no mode switch), or false to go back to a window. In an EXPORTED BUILD it applies this frame. In the EDITOR the window is the editor's, so it is left alone and the Console says so once — the same honesty app.quit() has about where it is running. For this session only; persist it with save.*." },
    ApiEntry { label: "app.setRetroIntegerScale", insert: "app.setRetroIntegerScale(", doc: "app.setRetroIntegerScale(true) — upscale the retro composite by a whole number and centre it, letterboxing the remainder. A fractional upscale puts some source rows on two screen pixels and some on three, which on pixel art with a small font is the difference between crisp and mush. For this session only; persist it with save.*." },
    ...
    ApiEntry { label: "mat.texture", insert: ".texture", doc: "mat.texture — the base-colour image, project-relative (\"art/shirt.png\"). Assigning swaps what the surface wears; \"\" clears it back to a flat colour. Reads back what you last set." },
    ApiEntry { label: "mat.normalMap", insert: ".normalMap", doc: "mat.normalMap — the surface's bump directions. \"\" clears it." },
    ApiEntry { label: "mat.roughnessMap", insert: ".roughnessMap", doc: "mat.roughnessMap — per-pixel roughness. \"\" clears it." },
    ApiEntry { label: "mat.metallicMap", insert: ".metallicMap", doc: "mat.metallicMap — per-pixel metalness. \"\" clears it." },
    ApiEntry { label: "mat.occlusionMap", insert: ".occlusionMap", doc: "mat.occlusionMap — baked ambient occlusion. \"\" clears it." },
    ApiEntry { label: "mat.color", insert: ".color", doc: "mat.color — the tint, MULTIPLIED into the texture: white leaves the picture alone, and a colour tints it. Takes a color(r, g, b) or any {r,g,b} table; reads back as a colour. The per-channel spellings mat.r / mat.g / mat.b are the same value, for animation lanes that key one number." },
    ApiEntry { label: "mat.emissive", insert: ".emissive", doc: "mat.emissive — light this surface gives off, scaled by emissiveStrength. A colour; the channels are also mat.emissiveR/G/B." },
    ApiEntry { label: "mat.specular", insert: ".specular", doc: "mat.specular — the colour of its highlight (channels: mat.specularR/G/B)." },
    ApiEntry { label: "mat.rim", insert: ".rim", doc: "mat.rim — the colour of the rim light around its silhouette (channels: mat.rimR/G/B)." },
    ApiEntry { label: "mat.alpha", insert: ".alpha", doc: "mat.alpha — opacity, 0..1. Also readable as mat.opacity." },
    ApiEntry { label: "mat.roughness", insert: ".roughness", doc: "mat.roughness — 0 is a mirror, 1 is chalk." },
    ApiEntry { label: "mat.metallic", insert: ".metallic", doc: "mat.metallic — 0 is a dielectric, 1 is bare metal. For a metal the ALBEDO is the reflection tint, so a black metal reflects nothing." },
    ApiEntry { label: "mat.emissiveStrength", insert: ".emissiveStrength", doc: "mat.emissiveStrength — how much light emissive gives off. 0 turns it off however bright the colour is." },
    ApiEntry { label: "mat.unlit", insert: ".unlit", doc: "mat.unlit — draw at full brightness, ignoring every light. Reads back as a BOOLEAN." },
    ApiEntry { label: "mat.fog", insert: ".fog", doc: "mat.fog — does the scene's fog reach this surface? false keeps a UI panel or a skybox plane out of the weather. Reads back as a BOOLEAN." },
    ApiEntry { label: "mat:setShaderParam", insert: ":setShaderParam(", doc: "mat:setShaderParam(\"glow\", 2.5) / mat:setShaderParam(\"nose\", x, y, z) — drive a .flsl uniform on THIS material every tick: the node's own for node:material(), ONE PART's override for node:material(\"Head#2\"). The node-level node:setShaderParam folds into the node's own Material and cannot reach a part, which is what this exists for — a character whose parts wear shaders (skin here, a face decal there) can now have a part's knobs driven at runtime. A GPU uniform write, never a recompile; unset lanes are 0. The write lands only on a part that already HAS an override wearing a .flsl — it never creates one (an override is a whole material, and a uniform must not blank a part), and a part with nowhere to put it is said once in the Console." },
    ApiEntry { label: "mat:setShaderTexture", insert: ":setShaderTexture(", doc: "mat:setShaderTexture(slot, ref) — point one of THIS material's .flsl texture slots somewhere else, at runtime — on one part of a model when the handle came from node:material(\"Head#2\"). `slot` is the name the shader declares (`texture face` -> \"face\"); `ref` is a project-relative image path, an `rt:<name>` render target, or \"\" to clear it. The character creator's face swap is this one line: node:material(\"Head#2\"):setShaderTexture(\"face\", \"faces/02.png\"). Same rule as setShaderParam: it lands on an existing override that wears a shader, and never creates one." },
    ApiEntry { label: "mat:shaderParam", insert: ":shaderParam(", doc: "mat:shaderParam(\"glow\") -> x, y, z, w — read a uniform back off this material: what you set this frame, else what the material carries. nil when it has no such knob. Reads in the same frame as the write, the way mat.color does." },
    ApiEntry { label: "mat:shaderTexture", insert: ":shaderTexture(", doc: "mat:shaderTexture(slot) -> ref — which image a texture slot points at: what you set this frame, else what the material carries; \"\" for a slot you cleared, nil for one nothing has named." },
    ApiEntry { label: "sp.flipX", insert: ".flipX", doc: "sp.flipX — mirrored left-to-right. The one line behind a character facing the way it walks: sp.flipX = mx > 0. Reads back as a BOOLEAN." },
    ApiEntry { label: "sp.flipY", insert: ".flipY", doc: "sp.flipY — mirrored top-to-bottom." },
    ApiEntry { label: "sp.cell", insert: ".cell", doc: "sp.cell — which cell of the Material's spritesheet draws, 0-based. An animation clip's Sprite ▸ frame lane writes this too, so a script that also sets it every frame is the one that wins." },
    ApiEntry { label: "sp.ppu", insert: ".ppu", doc: "sp.ppu — pixels per world unit, measured against ONE CELL of the sheet: it is what makes a 16-pixel tile exactly one unit wide. 0 means \"size me by `size` instead\"." },
    ApiEntry { label: "sp.size", insert: ".size", doc: "sp.size — the sprite's world edge length, used when ppu is 0. For art that is not pixel art." },
    ApiEntry { label: "sp.pivotX", insert: ".pivotX", doc: "sp.pivotX — where the node's origin sits across the sprite, 0..1 (0.5 = centred). Outside 0..1 is allowed: an origin off the sprite is a legitimate thing to want." },
    ApiEntry { label: "sp.pivotY", insert: ".pivotY", doc: "sp.pivotY — the origin up the sprite. 0 puts it at the sprite's feet, which is what a Y-sorted character wants; setting one axis leaves the other alone." },
    ...
    ApiEntry { label: "node:sound", insert: "node:sound()", doc: "node:sound() — the handle for this node's Audio Source component. :play() (restarts), :stop(), :pause(), :resume(), :setClip(\"audio/x.ogg\"), :seek(secs), :isPlaying(), :isLoading(), :position(). Tunables (volume/pitch/distances/…) live on node:getcomponent(\"AudioSource\")." },
    ...
    ApiEntry { label: "changed", insert: "changed", doc: "function changed(node) — UI hook: a text field's value changed (typing, paste, backspace), or a draggable slider's did (a drag or a click on the track). Once per frame however many keystrokes landed; never for a frame the slider was held still. Read node.text, or getComponent(\"UiSlider\").value." },
    ...
    ApiEntry { label: "ui.changed", insert: "ui.changed(", doc: "ui.changed(element) — a text field's or a draggable slider's value changed this frame. Read the value with element.text, or getComponent(\"UiSlider\").value." },
    ...
    ApiEntry { label: "cloud", insert: "cloud", doc: "Floptle Cloud read as the GAME, with the key in project.ron: cloud.avatar for a player's picture, cloud.config for your remote config, cloud.get for any public read under /games/ or /players/. Reads only; what a player does goes through account.*." },
    ApiEntry { label: "cloud.game", insert: "cloud.game()", doc: "cloud.game() -> string|nil — this project's game slug (what /games/<slug>/… addresses), or nil when the project is not connected to Floptle Cloud." },
    ApiEntry { label: "cloud.get", insert: "cloud.get(\"/games/\" .. cloud.game() .. \"/\", function(res)\n  \nend)", doc: "cloud.get(path, function(res) end) — read from Floptle Cloud as the game: the engine attaches the project's game key, so it works with nobody signed in. Paths under /games/… and /players/… only, GET only (a game key never writes). res as http.get, with json parsed. Play only, same rate limits as http.*. Refuses: any other path; a project not connected to Cloud." },
    ApiEntry { label: "cloud.avatar", insert: "cloud.avatar(", doc: "cloud.avatar(playerId [, size], function(tex, err) end) — a player's Foverse picture as a texture, playerId being net.identity(peer).id or account.player().id. size is 64, 128 (default) or 256; the picture is square. err is \"no_picture\" when the player has none: draw your own stand-in. tex works anywhere a texture path does; a UI image with radius = half its size draws round. Loaded once per session per player and size; assets.release(tex) lets it go. Refuses: a size other than 64/128/256; a project not connected to Cloud." },
    ApiEntry { label: "cloud.config", insert: "cloud.config(function(config, err)\n  \nend)", doc: "cloud.config(function(config, err) end) — the remote config you publish from the game's page on fopull.com: a table of your keys, {} when none are set, or nil and why. Changed on the page, live in every copy of the game without a new build. Refuses: a project not connected to Cloud." },
    ...
    ApiEntry { label: "camera.exists", insert: "camera.exists()", doc: "camera.exists() — true once a live game camera is being fed. Guard the other camera.* calls with it during the first frames, or while a scene without a camera is up. True throughout a `floptle shot`, which feeds its picture's size; never under `floptle run`, which draws nothing." },
    ...
    ApiEntry { label: "overlapSphere", insert: "overlapSphere(", doc: "overlapSphere(center, radius [, opts]) — everything inside a sphere, DEEPEST overlap first, as hit tables ({x,y,z, nx,ny,nz, distance, node, material}) — the same fields raycast returns. Reports static geometry AND body hulls. opts takes { exclude = node, layers = {\"Enemies\"} }. The blast-radius / \"what is in this area\" query." },
    ...
    ApiEntry { label: "steam", insert: "steam", doc: "Steam integration: identity, app/build info, and steam.available() for branching. Always present so steam.available() is always safe to call — nil (not an error) is what every other steam.* getter answers when it's false. See the Steam integration plan." },
    ...
    ApiEntry { label: "nav.agent", insert: "nav.agent(", doc: "nav.agent(node[, opts]) — make this node something that walks the navmesh, and get a handle to order about. THE call for \"move a unit from A to B\": agent:moveTo(point), and it finds its own way, goes round its neighbours, slows down at the end and stops. Options, all optional: speed, accel, radius, arrive (how close counts as there), slow (where it starts easing off), avoid (take other agents into account), priority (who gives way), separation, repath (seconds between route checks), giveUpAfter (seconds of no progress before it reports blocked), drive ('auto' | 'transform' | 'velocity' | 'none'), and filter = { avoid = {'water'}, cost = { mud = 0.5 }, canDrop = false, canJump = false }. canDrop and canJump refuse the drops and jumps the bake found for itself — a turret, a cart or anything that does not leave the floor says no here rather than needing its own bake. drive defaults to 'auto': a node with a physics body is steered through the body, one without has its transform moved. The whole crowd is stepped once a frame by the engine, after your update — you never call a step function. ON A PROCEDURAL OR STREAMED LEVEL THE NAVMESH ARRIVES AFTER start(): there is no geometry at all when start() runs, so nav.ready() is false, and asking for the agent once behind that check means it is never made and every routing call silently takes your fallback for the rest of the session. Ask every frame until you have one — a script that handles 'no navmesh yet' gracefully handles 'no navmesh ever' identically, which is why this fails quietly." },
    ApiEntry { label: "nav.agents", insert: "nav.agents()", doc: "nav.agents() — how many nav agents exist right now. For a HUD, a test, or checking that the ones you destroyed really went." },
    ApiEntry { label: "nav.budget", insert: "nav.budget(", doc: "nav.budget([n]) — how many path searches the whole crowd may run per frame (default 8); returns the current value, and sets it when given a number. A hundred units given one order do not all think on the same frame: they queue, oldest first, and keep walking their old route while they wait. Raise it for a game where a burst of orders should be acted on at once, lower it if the searches show up in a frame graph." },
    ApiEntry { label: "nav.link", insert: "nav.link(", doc: "nav.link(name | id[, open]) — open or shut a Nav Link, or ask whether it is open. nil when there is no link by that name. This is the door: nav.link('front gate', false) makes every route that used it repath, nothing is rebaked, and a unit already halfway across finishes crossing rather than stopping in mid-air." },
    ApiEntry { label: "nav.obstacle", insert: "nav.obstacle(", doc: "nav.obstacle(centre, size) — cut a box out of the baked navmesh, right now, and get a handle back. The crate dropped in a corridor: routes through that space stop existing, everything walking one repaths, and the level is not measured again. Hundreds of times cheaper than a rebake for a small thing on a big level — a 256 m level rebakes in ~460 ms and carves in ~0.6 ms. It is an OPTION and not a replacement: where the level genuinely changed shape (a building came down) the background rebake is the honest answer and this is not. The hole is grown outward to whole navmesh cells, so read ob.size rather than assuming you got the box you asked for. nil when the scene has no bake. There is deliberately no moving obstacle: carving every frame is rebuilding every frame, which is the cost this exists to avoid." },
    ApiEntry { label: "nav.rebake", insert: "nav.rebake(", doc: "nav.rebake(centre, size) — re-measure this box of the level and splice the answer into the navmesh, in the same frame. THE call for a level that builds itself: a streamer that has just finished a chunk, a generated room, a wall that came down. A full rebake measures the WHOLE level to account for one box, so the cost of building a chunk grows with how much level is already loaded — which is backwards, because the amount of new level per chunk is constant. This costs the box. Different from nav.obstacle: a crate standing on the floor is an obstacle and can be taken away again; a corridor that has just been built is a rebake and becomes the level. Carved obstacles survive it. It QUEUES like spawn (re-measuring needs the world's triangles, which the scripting side does not have) and lands in the same pass, after that pass's spawns and destroys — so build the chunk and ask in the same breath. World coordinates; the box is snapped outward to whole navmesh cells. Needs a navmesh already baked to splice into." },
    ApiEntry { label: "nav.obstacles", insert: "nav.obstacles()", doc: "nav.obstacles() — how many holes nav.obstacle has cut in the navmesh right now. Zero with no bake." },
    ApiEntry { label: "nav.clearObstacles", insert: "nav.clearObstacles()", doc: "nav.clearObstacles() — take every runtime obstacle away at once and give the whole level back, returning how many there were. For a wave ending or a level resetting, so nothing has to have kept a list of every crate." },
    ApiEntry { label: "obstacle", insert: "obstacle", doc: "A navmesh obstacle handle, from nav.obstacle(centre, size). Fields: id, active, position, size (the hole ACTUALLY cut, grown out to whole cells). One method: ob:remove(), which gives the ground back and returns false if it had already gone." },
    ApiEntry { label: "obstacle:remove", insert: "obstacle:remove()", doc: "obstacle:remove() — give the ground back where this obstacle was, and return whether it was still there. Everything that gave up on a route through it is un-blocked, so a unit that stopped beside the crate starts walking again. Calling it twice is false, not an error." },
    ApiEntry { label: "agent", insert: "agent", doc: "A nav agent handle, from nav.agent(node). It walks its node along the navmesh: agent:moveTo(point) and read agent.state as it goes. Everything about it is a field or a method on this handle — there is no per-frame step to call." },
    ApiEntry { label: "agent:moveTo", insert: "agent:moveTo(", doc: "agent:moveTo(point) — send it to a world point (a vec3, anything with x/y/z, or a node). Idempotent: ordering it to where it is already heading costs nothing, so calling this every frame to follow a moving target is fine. The point does not have to be exactly on the navmesh — it is snapped — and a point that is on the mesh but cut off makes the agent walk as near as it can and then report blocked." },
    ApiEntry { label: "agent:stop", insert: "agent:stop()", doc: "agent:stop() — cancel the order and stand still. Anything mid-crossing finishes the crossing first: halfway up a ladder is not a place to be left." },
    ApiEntry { label: "agent:teleport", insert: "agent:teleport(", doc: "agent:teleport(point) — put it AND its node somewhere without walking there, and forget what it was doing. For spawns, respawns and cutscenes. (With drive = 'none' the engine leaves the node alone, so move it yourself.)" },
    ApiEntry { label: "agent:set", insert: "agent:set{", doc: "agent:set{ ... } — change how it walks, mid-game. Takes the same options as nav.agent; anything left out is left alone. Slowing a unit down, making it stop giving way, or swapping its filter when it picks up a boat." },
    ApiEntry { label: "agent:corners", insert: "agent:corners()", doc: "agent:corners() — the corners still to walk, as a list of vec3 in world space. For drawing the route while working out why a unit went the way it did." },
    ApiEntry { label: "agent:destroy", insert: "agent:destroy()", doc: "agent:destroy() — take it out of the crowd. Not required (an agent whose node is destroyed goes with it on the next frame) but the right thing to call from a script's own teardown." },
    ApiEntry { label: "agent.state", insert: "agent.state", doc: "'idle' | 'moving' | 'arrived' | 'blocked' | 'crossing' — and 'gone' for a handle whose agent has been destroyed. 'blocked' is the one worth acting on: it means the goal cannot be reached from here, or the unit has made no progress for giveUpAfter seconds. A unit pinned by its own crowd rests and tries again on its own; one whose goal is genuinely cut off stays blocked until something changes. A unit standing still with no explanation is the commonest \"the pathfinding is broken\" report there is, and this is the explanation." },
    ApiEntry { label: "agent.arrived", insert: "agent.arrived", doc: "True once it got there. The flag to hang \"and then attack / gather / open the door\" off." },
    ApiEntry { label: "agent.moving", insert: "agent.moving", doc: "True while it still has somewhere to be — walking or crossing a link." },
    ApiEntry { label: "agent.blocked", insert: "agent.blocked", doc: "True when it cannot get there right now: unreachable, or no progress for giveUpAfter seconds. A crowd pin clears itself; a cut-off goal does not." },
    ApiEntry { label: "agent.offMesh", insert: "agent.offMesh", doc: "True when the order named a place the navmesh does not cover — as opposed to a place it cannot reach. Nearly always a Nav Mesh volume smaller than the level; ordering it somewhere else nearby will not help." },
    ApiEntry { label: "agent.complete", insert: "agent.complete", doc: "Whether the route it is walking actually reaches the order. False means it is heading for the closest it can get, which is the right behaviour and worth being able to say out loud — \"can't get there, going as near as I can\"." },
    ApiEntry { label: "agent.remaining", insert: "agent.remaining", doc: "How far there is left to walk, in metres, ALONG THE ROUTE rather than through the walls. The number an ETA or a progress bar wants." },
    ApiEntry { label: "agent.velocity", insert: "agent.velocity", doc: "How fast it is going, as a vec3. With drive = 'none' this is the whole point of the agent: it steers, and your script decides what that means for a vehicle, a boat or an animation." },
    ApiEntry { label: "agent.speed", insert: "agent.speed", doc: "How fast it is going along the ground, in units per second. What an idle/walk/run animation blend reads." },
    ApiEntry { label: "agent.pos", insert: "agent.pos", doc: "Where it is, in world space." },
    ApiEntry { label: "agent.target", insert: "agent.target", doc: "Where it was told to go, or nil if it has no order." },
    ApiEntry { label: "agent.link", insert: "agent.link", doc: "The name of the Nav Link being crossed right now, or nil the rest of the time. This is the hook for \"play the climb animation\": if agent.link == 'ladder' then ... end." },
    ApiEntry { label: "agent.linkProgress", insert: "agent.linkProgress", doc: "How far across a link it is, 0 to 1 — nil when it is not on one. What a vault or climb animation is driven by, so the animation and the movement cannot disagree." },
    ApiEntry { label: "agent.linkKind", insert: "agent.linkKind", doc: "What kind of crossing it is on: 'placed' for a Nav Link node somebody put there, or 'drop' / 'jump' for one the bake worked out from the shape of the floor. nil when it is not on one. This is the animation rule — a climb and a fall are not the same move, and a level with six hundred generated drops in it needs one rule rather than six hundred names." },
    ApiEntry { label: "agent.alive", insert: "agent.alive", doc: "False once the agent has been destroyed (or its node has). A handle kept in a variable answers about itself rather than pointing at whoever took its place." },
    ...
    ApiEntry { label: "nav.onMesh", insert: "nav.onMesh(", doc: "nav.onMesh(point[, tolerance]) — is this point on the walkable surface? The allocation-free version of nav.nearest, for the per-frame \"am I still on the floor\" check that does not want the point back. False when there is no navmesh at all, so it never raises." },
    ApiEntry { label: "nav.regionOf", insert: "nav.regionOf(", doc: "nav.regionOf(point[, tolerance]) — which walkable island a point is on, or nil if it is not on the navmesh. Two points in different regions can never be walked between, so comparing two ids rules out a search that was never going to succeed. The number itself means nothing beyond \"the same one is the same island\"." },
    ApiEntry { label: "nav.reachable", insert: "nav.reachable(", doc: "nav.reachable(from, to) — can something actually walk from here to there? Different from nav.path(...) ~= nil: a path that exists but only gets partway comes back with false alongside it, and this is that flag. Cheaper than a path when the yes-or-no is all you wanted." },
    ApiEntry { label: "nav.distance", insert: "nav.distance(", doc: "nav.distance(from, to) — how far it is to WALK, in metres, or nil if there is no complete route. This is the number a decision should be made on: the straight-line distance to something on the far side of a wall is a lie, and \"chase the nearest one\" built on it picks the wrong one every time." },
    ApiEntry { label: "nav.raycast", insert: "nav.raycast(", doc: "nav.raycast(from, to) — walk a straight line across the surface and get back where it stops, or nil if the whole line is walkable. The walker's answer rather than the collider's: a ledge this character would fall off is empty air to a physics ray and a wall to this. Use it to decide \"can I just walk at it\" before asking for a full path." },
    ApiEntry { label: "nav.random", insert: "nav.random(", doc: "nav.random(u, v[, near, radius]) — a point somewhere on the walkable surface, weighted by area so a big room is likelier than a corridor. The two numbers 0..1 are YOURS — call it as nav.random(math.random(), math.random()). That is deliberate: the engine rolls back and re-simulates, so a wander destination has to come from the same seeded stream as everything else the tick decided, and a navmesh that reached for its own randomness would desync every rollback that touched it. near and radius restrict it to a square neighbourhood (a square, not a circle — sampling a circle needs a re-draw, and there is no stream here to re-draw from)." },
    ApiEntry { label: "nav.sampler", insert: "nav.sampler(", doc: "nav.sampler([near, radius]) — the neighbourhood gathered ONCE, for many draws: local s = nav.sampler(node.position, 20); local p = s:point(math.random(), math.random()). nav.random re-gathers, re-sorts and re-measures every polygon its window covers on every call, so its cost grows with the radius — a dozen agents redrawing a destination in one frame was measured at 4 ms. A squad wanders around the same place, so hold the gather and each draw becomes a binary search. s.count is how many polygon parts the window covers; a count of 0 answers nil forever, which otherwise reads as bad luck. s:point(u, v) takes the same caller-supplied 0..1 pair nav.random does and for the same reason. A sampler is a SNAPSHOT: nav.splice re-baking the level does not reach one already built, so rebuild it when the ground changes." },
    ApiEntry { label: "nav.settings", insert: "nav.settings()", doc: "nav.settings() — the character the mesh was baked for: radius, height, maxSlope, stepHeight, cellSize, maxDrop (the tallest ledge it steps off), maxJump (the widest gap it clears), minRegionArea (the smallest patch of ground kept), plus areaCount and area (square metres of walkable ground). A script moving a body along a path needs the radius the mesh was eroded by; guessing it is how a character ends up scraping the wall the erosion existed to avoid." },
    ApiEntry { label: "nav.areas", insert: "nav.areas()", doc: "nav.areas() — every walkable area, as ONE FLAT ARRAY of numbers plus a count. Eleven numbers each, in nav.AREA_STRIDE steps: minX, minZ, maxX, maxZ, yMin, yMax, region, centreX, centreY, centreZ, ground — all world space, and ground is a one-based index into nav.ground(). Flat rather than a table per area on purpose: a real bake is thousands of areas, and a held Lua table costs one of a few thousand mlua slots, so a table each exhausts them and panics the editor rather than raising something a script could catch. One array costs one slot however big the level is. Read it as: local a, n = nav.areas(); for i = 0, n - 1 do local o = i * nav.AREA_STRIDE ... end" },
    ApiEntry { label: "nav.links", insert: "nav.links()", doc: "nav.links() — every portal between two areas, as one flat array plus a count. Eight numbers each, in nav.LINK_STRIDE steps: from, to, leftX, leftY, leftZ, rightX, rightY, rightZ. from and to are ONE-BASED indices into nav.areas(); left and right are the portal's endpoints as somebody walking from `from` into `to` sees them, so a smoother never has to work out which side of itself it is on. Each portal appears once per direction." },
    ApiEntry { label: "nav.AREA_STRIDE", insert: "nav.AREA_STRIDE", doc: "How many numbers nav.areas() uses per area (11). Read it rather than writing the number: fields are appended, never inserted, so code written against the constant keeps working." },
    ApiEntry { label: "nav.LINK_STRIDE", insert: "nav.LINK_STRIDE", doc: "How many numbers nav.links() uses per link (8)." },
    ApiEntry { label: "nav.ground", insert: "nav.ground()", doc: "nav.ground() — the kinds of ground this bake knows about, as { {name, cost}, ... } in the order an area's ground numbers them. These are the names a filter says: avoid = {'water'} means something only because the level called an area that, and this is how a script finds out which names the level offers instead of guessing at one and having a typo read as nothing to avoid. Tables rather than a flat array, unlike its neighbours — a level has a handful of these where it has thousands of areas, and a name cannot be a number. nil with no bake." },
    ApiEntry { label: "nav.offLinks", insert: "nav.offLinks()", doc: "nav.offLinks() — every off-mesh link in the level as data: { id, name, from, to, bidirectional, cost, duration, enabled, ground, kind, generated }, world space. kind is 'placed' for a Nav Link node somebody put there, or 'drop' / 'jump' for one the bake worked out from the shape of the floor; generated is the same fact as a boolean. Distinct from nav.links(), which is the thousands of portals the bake derived between neighbouring areas — two things called links is inherited and worth knowing before reading either. Use nav.link(name, open) to change one; this only reads. nil with no bake." },
    ...
    ApiEntry { label: "perf.scriptMs", insert: "perf.scriptMs(", doc: "perf.scriptMs(\"planet_walker\") — one script's own average cost, by file name. 0 for a script that has not run, which is different from an error. This is HOOK time: the per-script figures are a breakdown of part of perf.ms(\"scripts\"), not all of it, and the difference is what the engine spends reaching a hook." },
    ...
    ApiEntry { label: "spherecast", insert: "spherecast(", doc: "spherecast(origin, dir, radius, max [, opts]) — the first thing a moving BALL of that radius would hit, or nil. A raycast that can't slip through a gap narrower than the thing you are actually moving. The hit carries the same fields raycast's does, hit.material included — the usual ground check for a footstep." },
    ...
    ApiEntry { label: "sound:isLoading", insert: "sound:isLoading(", doc: "True from audio.play until its clip has been read in the background; it reads as playing meanwhile, and what you do to it (seek, pause, volume, stop) applies when it starts." },
    ...
    ApiEntry { label: "source:isLoading", insert: "source:isLoading(", doc: "True from play until its clip has been read in the background; it reads as playing, from 0 seconds, meanwhile." },
    ...
    ApiEntry { label: "vec2:magnitude", insert: "vec2:magnitude(", doc: "vec2:magnitude() — how long the 2-D vector is. The same call as vec2:length(), under the name most engines use for it." },
    ...
    ApiEntry { label: "vec3:magnitude", insert: "vec3:magnitude(", doc: "vec3:magnitude() — how long the vector is. The same call as vec3:length(), under the name most engines use for it — both are here so neither spelling is a dead end." },
...
Real vector VALUES with operators, not just x/y/z triplets:
  ...
  • v:length() (aka v:magnitude())  v:lengthSquared()  v:normalized()
...
Reference files under Assets/ in code, and swap a node's components at runtime.
  ...
  • assets.readJson(\"charts/a.json\")  a JSON file as a Lua value (nil, why on failure)
  • assets.writeJson(path, value)      write one back — a level editor built IN the game
  • assets.readText / writeText        the same for any text file
