...
// ---------------------------------------------------------------------------
// 6. Make it multiplayer — the platformer, on two machines
// ---------------------------------------------------------------------------

/// `scripts/platformerGame.lua` — hosting a session
pub(crate) const MP_GAME_LUA: &str = "\
-- The script that knows about the GAME rather than about a node: the score,
...
/// `scripts/platformerGame.lua` — the score in `synced`
pub(crate) const MP_GAME_2_LUA: &str = "\
-- The script that knows about the GAME rather than about a node: the score,
...
/// `scripts/coin.lua` — a pickup every machine agrees on
pub(crate) const NET_COIN_LUA: &str = "\
...
/// `scripts/netDoor.lua`
pub(crate) const NET_DOOR_LUA: &str = "\
...
const MULTIPLAYER: Tutorial = Tutorial {
    id: "multiplayer",
    title: "Make it multiplayer",
    tagline: "The platformer, played by two machines at once — and everything that has to change for that to work.",
...
",
    steps: &[
        Step {
            title: "Start from the platformer",
            body: "\
...
thing you hate about your game.",
            code: None,
            check: Check::Networked("Player"),
        },
        Step {
            title: "Host from the game script",
            body: "\
...
says which one you're looking at is worth more than any amount of reasoning.",
            code: Some(("platformerGame", MP_GAME_LUA)),
            check: Check::Contains {
                script: "platformerGame",
                needle: "net.host",
                what: "hosts a session",
            },
        },
        Step {
            title: "Join a ghost, then ruin the connection",
            body: "\
...
with someone saying \"it's weird on my end\".",
        ...
        Step {
            title: "The game itself is networked",
            body: "\
...
That's the whole step. The script that uses it is next.",
            code: None,
            check: Check::Networked("Game"),
        },
        Step {
            title: "Move the score into synced",
            body: "\
...
truth\" buys you.",
            code: Some(("platformerGame", MP_GAME_2_LUA)),
            check: Check::Contains {
                script: "platformerGame",
                needle: "replicated = { score",
                what: "declares the score as a synced value",
            },
        },
        Step {
            title: "Coins everyone agrees on",
            body: "\
...
once, not twice.",
            code: Some(("coin", NET_COIN_LUA)),
            check: Check::Contains {
                script: "coin",
                needle: "synced.taken = true",
                what: "marks the coin taken through synced",
            },
        },
        Step {
            title: "A door you ask the server to open",
            body: "\
...
Wrong for your own legs, which is the last step.",
            code: Some(("netDoor", NET_DOOR_LUA)),
            check: Check::NodeRuns { node: "Door", script: "netDoor" },
        },
        Step {
            title: "Make your own character predicted",
            body: "\
