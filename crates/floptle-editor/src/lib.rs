// **Without the editor half, the editor's own helpers are unreachable by
// construction** — not wrong, just not compiled into a game. Several hundred
// functions, consts and types are in that position, and gating each one would
// bury the seams that actually matter (the ones marked `#[cfg(feature =
// "editor-ui")]` below) under mechanical noise. So the player configuration
// allows it, and only it: the editor build keeps every warning it ever had,
// which is the build the workspace gate compiles.
#![cfg_attr(not(feature = "editor-ui"), allow(dead_code, unused_imports))]

//! # Floptle Editor
//!
//! The authoring application — an egui shell over a live wgpu viewport
//!. It renders the World **loaded from a `.ron` scene** with the
//! engine's PS1/retro look, and lets you select an object, move it, and save.
//!
...
//! editor binary (`floptle`), the headless verbs (`floptle run`, `floptle
//! shot`), and the standalone player (`floptle-player`), which builds this
//! crate with `--no-default-features` so no editor UI is compiled into a
//! shipped game at all.
//!
//! A second implementation of "play the game" would drift from the one the
//! editor shows you, and the difference would only ever be found by a player.

use std::collections::HashMap;
use std::path::{Path, PathBuf};
use std::sync::Arc;
use floptle_core::time::{Instant, SystemTime};
...
// three-line shim over it. See `player.rs` for why it lives in this crate.
pub mod player;
mod report;
#[cfg(feature = "editor-ui")]
mod run;
...
#[derive(Default)]
// Deferred commands the authoring UI raises and the frame applies. A build
// raises none of them: nothing in it can click.
#[cfg(feature = "editor-ui")]
struct EditorCmd {
    ...
    close_menu: bool,
    /// Flip the selection lock (the Inspector's 🔒). A command rather than a
    /// direct write because the Inspector draws from a context holding the
    /// selection by mutable reference, and the flag lives on the editor.
    toggle_selection_lock: bool,
    ...
    /// Open this file in the user's external editor.
...
struct EditorTabViewer<'a> {
    ...
    /// Whether the selection is held (`Editor::selection_locked`). Read-only
    /// here: the Hierarchy consults it before acting on a click, and the
    /// Inspector draws its state and asks for the flip through `cmd`.
...
fn distribution_version() -> String {
    // A web template is built from a tagged release, so there the compiled-in
    // version is the distribution's: no `version.json` to consult, and no
    // executable to look beside.
    #[cfg(not(target_arch = "wasm32"))]
    if let Some(v) = std::env::current_exe().ok().and_then(|exe| {
        let json = floptle_vfs::read_to_string(exe.with_file_name("version.json")).ok()?;
        json_string_field(&json, "version")
    }) {
        return v;
    }
    floptle_core::ENGINE_VERSION.to_string()
}
...
#[cfg(not(target_arch = "wasm32"))]
pub use player::run_player;

/// Run the editor application: parse the command line, then either serve a
/// headless verb and exit, or open the window and enter the frame loop.
///
/// The binary is a three-line shim over this so that the same code can be
/// reached as a library — see the module docs.
// The authoring application's entry point. The player's is `run_player`.
#[cfg(feature = "editor-ui")]
pub fn run() {
    ...
    // **Verbs first, flags after**. `cli::dispatch` claims the
...
struct Editor {
    ...
    ui_style_files: Vec<(std::path::PathBuf, Option<SystemTime>)>,
    ...
    terrain_flush_queue: Vec<(Entity, floptle_core::time::Instant)>,
    ...
    /// the rest of the scene — reading a value off another node, dragging a
    /// light to see the effect on the node you are tuning. Toggled from the
    ...
    /// the world takes one away (the node deleted, the scene switched).
    selection_locked: bool,
    ...
    input_map_mtime: Option<SystemTime>,
    ...
    /// The join-by-code buffer (a lobby code: six characters on Floptle
    /// Cloud, five on a self-hosted relay).
    ...
    /// A build's own system clipboard, kept alive for the session so a game's
    /// text fields can copy and cut. See `Editor::os_clipboard_set`.
    #[cfg(all(not(feature = "editor-ui"), not(target_arch = "wasm32")))]
    player_clipboard: Option<arboard::Clipboard>,
    ...
    dt_snap_error: f32,
    /// The Lua VM that runs node scripts in play mode.
    ...
    /// The external editor command for "Open in IDE"; a user preference.
...
mod migrate_tests {
    ...
    fn migrate_pins_an_existing_projects_vec3_and_keeps_an_explicit_one() {
        ...
        floptle_vfs::write(&cfg, "(retro: true, retro_height: 240)").unwrap();
        ...
        assert!(
            floptle_vfs::read_to_string(&cfg).unwrap().contains("script_vec3: Some(Exact)"),
